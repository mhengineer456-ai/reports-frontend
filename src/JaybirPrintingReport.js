...
export default function JaybirPrintingReport() {
    ...
    try {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        parsed.push({
          ...
          printingDate: row[dateIdx !== -1 ? dateIdx : 7] || "",
          totalPcs: pcs,
  ...
  const exportToCSV = () => {
    ...
    const headers = ["Timestamp", "Lot Number", "Garment Type", "Fabric", "Style", "Brand", "Supervisor", "Printing Date", "Total Pcs", "WIP Remarks", "Status", "Completion Date"];
        ...
        <div className="table-card">
          <div className="table-header">
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              <span style={{ fontWeight: 800, fontSize: "1.05rem" }}>Jaybir Printing Lots</span>
                ...
                <tbody>
                  {filteredData.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td style={{ fontWeight: 700 }}>{item.printingDate || "-"}</td>
                      <td>
