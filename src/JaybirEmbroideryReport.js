...
export default function JaybirEmbroideryReport() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const [embroideryRes, jobOrderRes] = await Promise.allSettled([
      ...
      if (
        embroideryRes.status !== "fulfilled" ||
        !embroideryRes.value?.ok ||
        !Array.isArray(embroideryRes.value.values) ||
        embroideryRes.value.values.length === 0
      ) {
        setData([]);
        setLoading(false);
        return;
      ...
      const rows = embroideryRes.value.values;
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const embDate = row[dateIdx !== -1 ? dateIdx : 7] || "";
        const ts = row[tsIdx !== -1 ? tsIdx : 0] || "";

        // Parse completion JSON / Date
        const compData = parseCompletionData(compVal);
        const agingDays = calculateAgingDays(embDate, ts, compData.completedDateObj, compData.isCompleted);
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      // ================= SHEET 1: MASTER EMBROIDERY LOTS =================
      const ws1 = workbook.addWorksheet("Embroidery Lots", {
      ...
      titleCell.value = "FACTORY SUITE PRO - JAYBIR EMBROIDERY REPORT";
      ...
      const filterItems = [
        ...
        ["Embroidery Date Filter", dateFilter || "All Dates"],
      ...
      saveAs(new Blob([buffer]), `Jaybir_Embroidery_Report_${ts}.xlsx`);
    } catch (e) {
  ...
  const exportToCSV = () => {
    ...
    const headers = ["Sr No.", "Timestamp", "Lot Number", "Garment Type", "Fabric", "Style", "Brand", "Season", "Section", "Party Name", "Direct Stitching", "Supervisor", "Embroidery Date", "Total Pcs", "Aging (Days)", "WIP Remarks", "Status", "Completion Date"];
  ...
  const handleDownloadPDF = () => {
    ...
    try {
      ...
      doc.text("FACTORY SUITE PRO - JAYBIR EMBROIDERY REPORT", pageW / 2, 30, { align: "center" });
      ...
      for (let p = 1; p <= totalPages; p++) {
        ...
        doc.text("Factory Suite Pro - Jaybir Embroidery Department", startX, pageH - 10);
        doc.text(`Page ${p} of ${totalPages}`, pageW - startX - 45, pageH - 10);
      }

      const fileDate = new Date().toISOString().slice(0, 10);
      const fileName = `Jaybir_Embroidery_Report_${fileDate}.pdf`;
      doc.save(fileName);
    } catch (err) {
      ...
      <div className="jaybir-report-container">
        ...
        <div className="report-header-box">
          <div>
            ...
            <p style={{ margin: "6px 0 0 0", color: "#ddd6fe", fontSize: "0.85rem" }}>
        ...
        <div className="table-card">
          <div className="table-header">
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              <span style={{ fontWeight: 800, fontSize: "1.05rem", color: "#0f172a" }}>Jaybir Embroidery Lots</span>
              <span style={{ background: "#ede9fe", padding: "3px 10px", borderRadius: "6px", fontSize: "0.8rem", fontWeight: 800, color: "#6d28d9" }}>
              ...
              <table className="data-table">
                ...
                <tbody>
                  {filteredData.map((item, idx) => (
                    <tr key={item.id}>
                      ...
                      <td style={{ fontWeight: 700, whiteSpace: "nowrap" }}>{item.embroideryDate || "—"}</td>
                      <td>
                        {item.agingDays !== null ? (
                          <span
                            style={{
                ...
                <tfoot>
                  <tr>
                    ...
                    <td></td>
                    <td></td>
                    <td style={{ fontWeight: 900, color: "#6d28d9" }}>{totalPcs.toLocaleString()}</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
