...
const DailyFoldingReport = () => {
 ...
 const fetchData = async () => {
  ...
  const getLotStatus = (record) => {
    const packingComplete = record['Folding Complete'] || '';
    const wipPacking = record['WIP Folding'] || '';
  ...
  const calculateAging = (record) => {
    try {
      const packingDate = record['Folding Date'];
      ...
      if (packingComplete.trim() && packingComplete !== '[]') {
        try {
          ...
          console.error('Error parsing Folding Complete date:', e);
          endDate = today;
        }
      }
  ...
  // Get recent remarks from WIP Folding data
  // Get recent remarks from WIP Folding data
const getRecentRemarks = (record) => {
  try {
    ...
    // For completed lots, get remarks from Folding Complete
    const packingComplete = record['Folding Complete'] || '';
    if (packingComplete.trim() && packingComplete !== '[]') {
      try {
        const statusData = JSON.parse(packingComplete);
        ...
        console.error('Error parsing Folding Complete remarks:', e);
      }
    }
  ...
  const getLatestStatusDetails = (record) => {
    try {
      ...
      // Check WIP Folding first
      const wipPacking = record['WIP Folding'] || '';
      if (wipPacking.trim() && wipPacking !== '[]') {
        statusData = JSON.parse(wipPacking);
      }
      
      // If no WIP data, check Folding Complete
      if (statusData.length === 0) {
        const packingComplete = record['Folding Complete'] || '';
        if (packingComplete.trim() && packingComplete !== '[]') {
  ...
  const exportToExcel = () => {
    const exportColumns = [
      ...
      'Folding Date', 'Folding Supervisor', 'Aging', 'Status', 'Recent Remarks', 
      'Stiching Supervisor'
    ];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "foldingReport");
    XLSX.writeFile(workbook, "FoldingReport.xlsx");
  };
  ...
  const exportToPDF = () => {
    ...
    const drawHeader = () => {
      ...
      const title = filters.holdLots ? 'FOLDING HOLD LOTS REPORT' : 'FOLDING ALLOTED REPORT';
      doc.setFontSize(18);
    ...
    // Helper function to extract date from timestamp or Folding Date
    const extractDate = (record) => {
      ...
      // Fallback to Folding Date
      return record['Folding Date'] || '';
    };
    ...
    const supervisorSummary = filteredData.reduce((acc, item) => {
      const supervisor = item['Folding Supervisor'] || 'Unassigned';
    ...
    const columnWidths = {
      ...
      4: 23,   // Brand - 23mm
      5: 15,   // Pcs - 15mm
      6: 25,   // Issue Date - 25mm (Changed header from Folding Date)
    ...
    // CHANGED: "Folding Date" to "ISSUE DATE" in header
    const headers = [
      [
        ...
        // CHANGED: Header text from "Folding Date" to "ISSUE DATE"
    ...
    // Note: Still using 'Folding Date' data but header shows as 'Issue Date'
    const body = filteredData.map(item => [
      ...
      // Data still comes from 'Folding Date' field
      { content: item['Folding Date'] || 'N/A', styles: { cellWidth: columnWidths[6], fontSize: 9, halign: 'center' } },
      { content: item['Folding Supervisor'] || 'N/A', styles: { cellWidth: columnWidths[7], fontSize: 9, halign: 'center' } },
      { 
      ...
      { content: item['Stiching Supervisor'] || 'N/A', styles: { cellWidth: columnWidths[10], fontSize: 9, halign: 'center' } }
    ]);
    ...
    autoTable(doc, {
      startY: y,
      head: [[
        { content: 'Folding Supervisor', styles: { halign: 'center', fontStyle: 'bold', cellWidth: summaryColumnWidths[0] } },
    ...
    const fileName = filters.holdLots 
      ? `folding_Hold_Lots_${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}.pdf`
      : `folding_Report_${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}.pdf`;
    doc.save(fileName);
  };
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="row-details-grid">
                ...
                <div className="detail-card">
                  <div className="detail-label">Folding Date</div>
                  <div className="detail-value">{selectedRow['Folding Date']}</div>
                </div>
                <div className="detail-card">
                ...
                <div className="detail-card">
                  <div className="detail-label">Folding Supervisor</div>
                  <div className="detail-value">{selectedRow['Folding Supervisor']}</div>
                </div>
                <div className="detail-card">
  ...
  >
    ←
          ...
          {/* Summary Cards - CHANGED: Total Manpower card replaced with Total Folding Supervisor card */}
        ...
        <div className="filters-container">
          ...
          <div className="filters-grid">
            ...
            <div className="filter-group">
              <label className="filter-label">Folding Supervisor</label>
              <select
              ...
              >
                ...
                {getUniqueValues('Folding Supervisor').map((supervisor, index) => (
                ...
                filteredData.map((item, index) => {
                  ...
                  return (
                    <tr key={index} onClick={() => openRowDetails(item)}>
                      ...
                      <td>{item['Folding Date']}</td>
                      <td><span className="supervisor-tag">{item['Folding Supervisor']}</span></td>
                      <td>
                        <span 
                ...
                <tr>
                  <td colSpan="12">
                    <div className="no-data">
                      ...
                      No folding lots match your current filters.
                    </div>
                  </td>
                </tr>
              )}
