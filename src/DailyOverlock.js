...
const DailyOverlockReport = () => {
 ...
 const fetchData = async () => {
  ...
  const getLotStatus = (record) => {
    const packingComplete = record['Overlock Complete'] || '';
    const wipPacking = record['WIP Overlock'] || '';
  ...
  const calculateAging = (record) => {
    try {
      const packingDate = record['Overlock Date'];
      ...
      if (packingComplete.trim() && packingComplete !== '[]') {
        try {
          ...
          console.error('Error parsing Overlock Complete date:', e);
          endDate = today;
        }
      }
  ...
  // Get recent remarks from WIP Overlock data
  // Get recent remarks from WIP Overlock data
const getRecentRemarks = (record) => {
  try {
    ...
    // For completed lots, get remarks from Overlock Complete
    const packingComplete = record['Overlock Complete'] || '';
    if (packingComplete.trim() && packingComplete !== '[]') {
      try {
        const statusData = JSON.parse(packingComplete);
        ...
        console.error('Error parsing Overlock Complete remarks:', e);
      }
    }
  ...
  const getLatestStatusDetails = (record) => {
    try {
      ...
      // Check WIP Overlock first
      const wipPacking = record['WIP Overlock'] || '';
      if (wipPacking.trim() && wipPacking !== '[]') {
        statusData = JSON.parse(wipPacking);
      }
      
      // If no WIP data, check Overlock Complete
      if (statusData.length === 0) {
        const packingComplete = record['Overlock Complete'] || '';
        if (packingComplete.trim() && packingComplete !== '[]') {
  ...
  const exportToExcel = () => {
    const exportColumns = [
      ...
      'Overlock Date', 'Overlock Supervisor', 'Aging', 'Status', 'Recent Remarks', 
      'Stiching Supervisor'
    ];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "OverlockReport");
    XLSX.writeFile(workbook, "Overlock.xlsx");
  };
  ...
  const exportToPDF = () => {
    ...
    const drawHeader = () => {
      ...
      const title = filters.holdLots ? 'OVERLOCK HOLD LOTS REPORT' : 'OVERLOCK ALLOTED REPORT';
      doc.setFontSize(18);
    ...
    // Helper function to extract date from timestamp or Overlock Date
    const extractDate = (record) => {
      ...
      // Fallback to Overlock Date
      return record['Overlock Date'] || '';
    };
    ...
    const supervisorSummary = filteredData.reduce((acc, item) => {
      const supervisor = item['Overlock Supervisor'] || 'Unassigned';
    ...
    const columnWidths = {
      ...
      4: 23,   // Brand - 23mm
      5: 15,   // Pcs - 15mm
      6: 25,   // Issue Date - 25mm (Changed header from Overlock Date)
    ...
    // CHANGED: "Overlock Date" to "ISSUE DATE" in header
    const headers = [
      [
        ...
        // CHANGED: Header text from "Overlock Date" to "ISSUE DATE"
    ...
    // Note: Still using 'Overlock Date' data but header shows as 'Issue Date'
    const body = filteredData.map(item => [
      ...
      // Data still comes from 'Overlock Date' field
      { content: item['Overlock Date'] || 'N/A', styles: { cellWidth: columnWidths[6], fontSize: 9, halign: 'center' } },
      { content: item['Overlock Supervisor'] || 'N/A', styles: { cellWidth: columnWidths[7], fontSize: 9, halign: 'center' } },
      { 
    ...
    autoTable(doc, {
      startY: y,
      head: [[
        { content: 'Overlock Supervisor', styles: { halign: 'center', fontStyle: 'bold', cellWidth: summaryColumnWidths[0] } },
    ...
    const fileName = filters.holdLots 
      ? `Overlock_Hold_Lots_${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}.pdf`
      : `Overlock_Report_${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}.pdf`;
    doc.save(fileName);
  };
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="row-details-grid">
                ...
                <div className="detail-card">
                  <div className="detail-label">Overlock Date</div>
                  <div className="detail-value">{selectedRow['Overlock Date']}</div>
                </div>
                <div className="detail-card">
                ...
                <div className="detail-card">
                  <div className="detail-label">Overlock Supervisor</div>
                  <div className="detail-value">{selectedRow['Overlock Supervisor']}</div>
                </div>
                <div className="detail-card">
  ...
  >
    ←
          ...
          {/* Summary Cards - CHANGED: Total Manpower card replaced with Total Overlock Supervisor card */}
          <div className="summary-cards">
            ...
            <div className="summary-card total-supervisors">
              <div className="card-label">
                <span>👤</span> Overlock Supervisors
              </div>
        ...
        <div className="filters-container">
          ...
          <div className="filters-grid">
            ...
            <div className="filter-group">
              <label className="filter-label">Overlock Supervisor</label>
              <select
              ...
              >
                ...
                {getUniqueValues('Overlock Supervisor').map((supervisor, index) => (
